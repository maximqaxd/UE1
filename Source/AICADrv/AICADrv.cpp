...
void UAICAAudioSubsystem::RegisterSound( USound* Sound )
{
	...
	if( Sound->Handle )
	{
		debugf( NAME_Log, "Sound %s already has handle, skipping", Sound->GetName() );
		return;
	}

	if( Sound->Data.Num() == 0 )
	{
		debugf( NAME_Warning, "Sound %s has no data", Sound->GetName() );
		return;
	}
...
UBOOL UAICAAudioSubsystem::PlaySound( AActor* Actor, INT Id, USound* Sound, FVector Location, FLOAT Volume, FLOAT Radius, FLOAT Pitch )
{
	...
	Voice->Actor = Actor;
	Voice->Looping = Sound->Looping || SOUND_SLOT_IS( Id, SLOT_Ambient );
	Voice->Sound = Sound;
