...
DEST_SYSTEM_DIR="${REPO_ROOT}/gamedata/System"
...
if [[ ! -f "${OUTPUT_DIR}/DCUtil.bin" ]]; then
  echo "DCUtil.bin was not produced in ${OUTPUT_DIR}" >&2
  exit 1
fi

cp "${OUTPUT_DIR}/DCUtil.bin" "${DEST_SYSTEM_DIR}/"
